...
for( T in seq(30,300,10) ){
  file<-paste0("/home/garofalo/analysis/chemHMC/Argon/Ar-450/cutoff100/",T,"K/HMC_hystory_",T,"K.txt")
  if(file.exists(file)){
    cat("### T=",T,"K\n\n")
...
save(dfall_c100, file = "Ar-450_HMC_cutoff100.Rdata")
...
count<-1
for( T in seq(30,300,10) ){
  file<-paste0("/home/garofalo/analysis/chemHMC/Argon/Ar-450/cutoff100/hot_start/",T,"K/HMC_hystory_",T,"K.txt")
  if(file.exists(file)){
    cat("### T=",T,"K\n\n")
    dft<-data.frame("obs"=c("H","K","V","dH","exp(-dH)"), "ave"=rep(1,5), "tauint"=rep(1,5) )
    df<-read.table(file, header = TRUE)
...
save(dfall_c100_h, file = "Ar-450_HMC_cutoff100_hot_start.Rdata")
